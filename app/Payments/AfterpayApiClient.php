...
class AfterpayApiClient {
	...
	{   
		try {
			...
			$response = $client->request('POST', $this->url . 'orders', [ 'headers' => $this->config, 'body' => $data, 'http_errors' => false]);
			if (!empty($response)) {                
	...
	{ 
		try {
			...
			$response = $this->client->post($this->url . 'payments/capture', ['headers' => $this->config , 'form_params' => $data, 'http_errors' => false])->getBody();
            if (!empty($response)) {                
                return $response;
            }
