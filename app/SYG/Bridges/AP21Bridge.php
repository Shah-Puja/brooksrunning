...
class AP21Bridge implements BridgeInterface {
    ...
    public function processPerson($data) {
        ...
        try {
            $response = $this->apiClient->post('Persons/?countryCode=AUFITs', ['body' => $data, 'http_errors' => false]);
            if ($response->getStatusCode() == 200) {
                return $response;
            }
        } catch (RequestException $e) {
    ...
    public function processOrder($PersonId, $data) {
        try {
            $response = $this->apiClient->post('Persons/' . $PersonId . '/Orders/?countryCode=AUFITs', ['body' => $data, 'http_errors' => false]);
            if ($response->getStatusCode() == 200) {
                return $response;
            }
        } catch (RequestException $e) {
