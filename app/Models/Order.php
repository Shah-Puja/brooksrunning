...
class Order extends Model {
    ...
    protected $fillable = ['order_no', 'cart_id', 'user_id', 'total', 'freight_cost', 'delivery_type', 'grand_total', 'status', 'payment_status', 'afterpay_token', 'transaction_id', 'transaction_status'];
    ...
    public static function createNew($cart, $user_id, $validatedAddress) {
        ...
        Order_address::updateOrCreate(
                ['order_id' => $order_id], Order_address::prepareAddressForSaving($validatedAddress)
        );
        ...
        $cart->cartItems->each(function($item) use ($order) {
            ...
            $order->orderItems()->create([
                ...
                'ap21_discount_xml' => ($item->ap21_discount_xml != '') ? $item->ap21_discount_xml : "",
