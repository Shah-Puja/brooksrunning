...
if (!function_exists('check_state_and_update_delivery_option')) {
    ...
    function check_state_and_update_delivery_option() {
        ...
        if (isset($cart->order->address->s_state) && $cart->order->address->s_state == 'New Zealand') {
            ...
            Cart::where('id', session('cart_id'))->update(['delivery_type' => $delivery_option, 'freight_cost' => $freight_charges, 'grand_total' => $grand_total])->limit(1);
            Order::where('cart_id', session('cart_id'))->update(['delivery_type' => $delivery_option, 'freight_cost' => $freight_charges, 'grand_total' => $grand_total])->limit(1);
        }
    }
